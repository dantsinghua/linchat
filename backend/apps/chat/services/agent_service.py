...
def _finalize_message(
    msg: Message, content: str, status: int, duration_ms: int,
    prompt_tokens: int, completion_tokens: int,
) -> None:
    """更新 assistant 消息最终状态（中断/完成共用）"""
    msg.content = content
    msg.status = status
...
def _finalize_execution(
    execution: LangGraphExecution, status: str, end_time, duration_ms: int,
        ...
        try:
            ...
            async with create_chat_agent() as agent:
                ...
                if interrupted:
                    full_response += "[已中断]"
                    _finalize_message(
                        assistant_msg, full_response, Message.STATUS_INTERRUPTED,
                    ...
                    yield StreamChunk(
                        type="interrupted", content="[已中断]", message_id=assistant_msg.message_id,
                    )
                else:
                    _finalize_message(
                        assistant_msg, full_response, Message.STATUS_NORMAL,
                        duration_ms, total_prompt_tokens, total_completion_tokens,
                    )
                    await message_repo.update(assistant_msg)
                    ...
                    async for event in agent.astream_events(
                        ...
                        if stop_event.is_set():
                            ...
                            await message_repo.update_content_and_status(
                                ...
                                full_response, Message.STATUS_INTERRUPTED,
                            )
                            yield StreamChunk(
                                type="interrupted", content="[已中断]",
