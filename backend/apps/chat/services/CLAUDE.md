...
| `generation.py` | **实际实现** | 活跃生成管理（register/unregister/signal_stop）；map_llm_exception 从 `apps.common.exceptions` 导入 |
| `context_service.py` | 兼容层 | 实际在 `apps.graph.services.context_service` |
| `inference_service.py` | 兼容层 | 实际在 `apps.graph.services.inference_service` |
| `gpu_lock.py` | 兼容层 | 实际在 `apps.graph.services.gpu_lock` |
| `media_service.py` | 兼容层 | 实际在 `apps.media.services.upload` |
| `minio_service.py` | 兼容层 | 实际在 `apps.common.storage.minio_service` |
| `document_parse_service.py` | 兼容层 | 实际在 `apps.media.services.document` |
...
| 方法 | 说明 |
|------|------|
| `send_message()` | 参数校验 -> 生成 request_id -> AgentService.execute() -> yield StreamChunk |
| `stop_generation()` | signal_stop(request_id) 发送停止信号 |
| `resume_generation()` | 校验消息状态 STATUS_INTERRUPTED -> AgentService.resume() |
| `reconnect_stream()` | 轮询增量内容（0.5s 间隔，最长 5 分钟） |

### HistoryService (chat_service.py)

| 方法 | 说明 |
|------|------|
| `load_messages()` | 游标分页查询，返回 MessageVO 列表 |
| `get_generating_message()` | 查找用户当前生成中的 assistant 消息 |
...
| 函数 | 说明 |
|------|------|
| `register_generation()` | 注册新生成，返回 stop_event |
| `unregister_generation()` | 取消注册 |
| `get_stop_event()` | 获取停止事件 |
| `signal_stop()` | 发送停止信号 |
...
| `MessageVO` | 消息视图对象，`from_entity()` 从 ORM 实体转换（含 attachments） |
| `InferenceTask` | 推理任务状态（Redis 临时存储），`to_json()/from_json()` 序列化 |
| `_get_tool_model_name()` | 异步获取激活的工具模型名称 |
...
3. ChatService 所有方法为 `@staticmethod`，通过类直接调用
