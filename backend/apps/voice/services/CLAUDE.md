...
| `voice_pipeline.py` | 175 | 语音推理管道编排（Agent→TTS + 持久化 + barge-in 打断（超时直接跳过） + 推理任务清理 + HA 音箱 TTS 路由 + 语音模式纯口语 Prompt 前缀） | 无（静态方法 + 类方法） |
...
| `speaker_service.py` | 104 | 声纹注册/删除/识别（对接 Gateway HTTP /v1/voice/speakers）；diarize 功能已废弃（DEPRECATED） | `speaker_service` |
| `device_service.py` | 76 | 设备注册/Token 管理（SM4 加密/解密匹配） | `device_service` |
| `response_decision_service.py` | 184 | 唤醒词检测 + 响应决策（RESPOND/RECORD_ONLY/STOP）+ LLM 意图分类（仅 ambient，含上下文增强）；超时安全降级 RECORD_ONLY | `response_decision_service` |
| `utterance_aggregator.py` | 97 | 多轮话语聚合器（缓冲 ASR 转录 → 静默超时/满缓冲 → 聚合回调） | 无（每个 ambient Consumer 创建） |
| `tts_router.py` | 167 | 跨设备 TTS 路由（Channels group_send 广播音频帧到浏览器连接）+ HA 音箱 TTS 输出（xiaomi_miot 直传文本，降级 media_player.play_media）+ 降级通知 | 无（每次 ambient pipeline 创建） |
...
VoiceConsumer（consumers.py + 3 Mixin）
  ...
  │     ├── message_repo（apps.chat）— 最近 5 条对话（LLM 上下文增强）
  │     └── MemoryService（apps.memory）— 用户记忆召回（LLM 上下文增强）
  ...
  │     ├── [ambient] TTSRouter — Channels group_send 跨设备 TTS 广播 + HA 音箱播报
  │     │     ├── [ha_speaker] xiaomi_miot.intelligent_speaker（优先，直传文本）
  │     │     ├── [ha_speaker 降级] TTSStreamClient → MinIO → media_player.play_media
  │     │     └── [ha_speaker 不可达] send_warning() 降级通知
  │     ├── voice_persist_service — PCM→WAV + MinIO 上传 + record_only_ambient
  │     ├── voice_session_service — 音频缓存读取/清理
  │     ├── voice_settings_repo — VoiceSettings（tts_output_device/ha_speaker_entity_id）
...
ASR transcription.completed
  ...
  → [voice_chat] 直接进入
  → _run_inner():
    → barge-in 检查: 旧 pipeline 正在运行 → cancel + 等待锁释放（2s 超时，超时则跳过本次 pipeline）
    ...
    → [语音模式] 包装 voice_text（纯口语前缀 + 禁止 Markdown）
    → response.start → AgentService.execute(user_message=voice_text) 流式
    ...
    → InferenceService.complete_task()（清理推理任务注册）
    → wait_idle() → shutdown()
    → [ambient] TTSRouter.send_control(user_id, "tts.completed")
    → [ambient + ha_speaker] _try_ha_speaker_tts() → TTSRouter.send_to_ha_speaker()
      → 优先 xiaomi_miot.intelligent_speaker 直传文本
      → 降级 TTS WAV → MinIO → media_player.play_media
      → 不可达 send_warning() 降级通知到浏览器
    → response.end
...
| `record_only_ambient(user_id, text)` | ambient RECORD_ONLY 消息保存（显式 `created_time=timezone.now()`）+ 超限 20 条自动清理（按 `created_time` 排序） |
...
| **4** | **LLM 意图分类** | **视置信度** | **仅 ambient + VOICE_DECISION_USE_LLM=True**；httpx JSON mode，超时由 VOICE_DECISION_LLM_TIMEOUT 控制；置信度 ≥ VOICE_DECISION_LLM_THRESHOLD 采用，否则穿透；**上下文增强**：注入最近 5 条对话 + 用户记忆（`_fetch_intent_context`）；**超时安全降级**：LLM 超时返回 RECORD_ONLY（confidence=1.0），不穿透到规则链；Prompt 模板: `voice_intent_classify.j2` |
...
跨设备 TTS 路由，通过 Django Channels `group_send` 将 TTS 音频帧广播到浏览器连接。解决 ambient 模式 ESP 设备推理 → 浏览器播放 TTS 的跨连接问题。016 新增 HA 音箱 TTS 输出能力。

| 方法 | 说明 |
|------|------|
| `group_name(user_id)` | 静态方法，返回 `voice_tts_{user_id}` |
| `send_binary(user_id, data)` | group_send 发送 `tts_audio_frame` 类型音频帧 |
| `send_control(user_id, event_type, payload?)` | group_send 发送 `tts_control` 类型控制消息（tts.started/tts.completed/warning） |
| `send_warning(user_id, reason, message)` | group_send 推送降级通知（HA 音箱不可达时） |
| `get_on_audio_callback(user_id)` | 返回闭包，可直接传给 TTSPipelineManager(on_audio=...) |
| `send_to_ha_speaker(entity_id, text)` | HA 音箱 TTS 播报：优先 xiaomi_miot.intelligent_speaker 直传文本；404 降级为 TTS WAV → MinIO → media_player.play_media；不可达抛 HASpeakerError |
| `_generate_tts_wav(text)` | 静态方法，调用 Gateway TTS 生成 WAV 音频（仅 HA 降级路径使用） |

**HASpeakerError**: HA 音箱不可达异常（ConnectionError/Timeout/HTTP 5xx），由 VoicePipeline._try_ha_speaker_tts() 捕获并降级。

**HA 音箱 TTS 降级链**:
1. xiaomi_miot.intelligent_speaker（直传文本，零延迟）
2. Gateway TTS → PCM→WAV → MinIO 上传 → media_player.play_media（Nginx 代理局域网 URL）
3. 完全不可达 → send_warning → 浏览器已有 TTS 音频（TTSPipelineManager 已处理）
