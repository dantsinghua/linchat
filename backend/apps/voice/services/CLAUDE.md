...
| `voice_pipeline.py` | 语音推理管道编排（ASR→Agent→TTSPipelineManager + 持久化 + barge-in 打断） | 无（静态方法 + 类方法） |
...
| `response_decision_service.py` | 唤醒词检测 + 响应决策（RESPOND/RECORD_ONLY/STOP）+ LLM 意图分类（仅 ambient 模式） | `response_decision_service` |
| `utterance_aggregator.py` | 多轮话语聚合器（缓冲 ASR 转录 → 静默超时/满缓冲 → 聚合回调） | 无（每个 ambient Consumer 创建） |
| `tts_router.py` | 跨设备 TTS 路由（Channels group_send 广播音频帧到浏览器连接） | 无（每次 ambient pipeline 创建） |
...
ASR transcription.completed
  → [ambient] EventMixin._handle_ambient_transcription()
    ...
    → _on_utterance_aggregated() → ResponseDecisionService.decide()
      ...
      → RECORD_ONLY: record_only_ambient() (保存+清理超限消息)
      → STOP: cancel()
  → [voice_chat] 直接进入
  → _run_pipeline_inner():
...
| **4** | **LLM 意图分类**（014 新增） | **视置信度** | **仅 ambient 模式 + VOICE_DECISION_USE_LLM=True**；httpx → DeepSeek JSON mode，1s 超时；置信度 ≥ VOICE_DECISION_LLM_THRESHOLD 采用，否则穿透 |
...
## UtteranceAggregator（utterance_aggregator.py）— 014 新增
...
## TTSRouter（tts_router.py）— 014 新增
