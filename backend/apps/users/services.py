...
def _convert_audio_to_wav16k(audio_data: bytes) -> bytes:
    """将任意音频格式（webm/opus/mpeg 等）转换为 WAV PCM16 16kHz mono。

    Gateway 声纹注册要求 WAV PCM16 16kHz mono 格式。
    前端 MediaRecorder 通常输出 webm/opus，需要后端转换。
    """
...
class AuthService:
    ...
    async def login(
        ...
        if not verify_password(decrypted, user.password_hash):
            ...
            raise AuthFailedException("用户名或密码错误")
        if user.is_guest_expired():
            logger.warning(
                "过期访客 %s 尝试登录", user.username
            )
            raise AuthFailedException("账号已过期")
...
class MemberService:
    ...
    async def create_member(
        ...
        """原子创建成员：校验用户名 → Gateway 声纹注册 → 事务内创建用户+SpeakerProfile

        Args:
            username: 用户名
            password_encrypted: SM4 加密的密码
            member_type: 成员类型（member/guest）
            audio_file: Django UploadedFile 声纹录音文件
            created_by_user_id: 操作人用户 ID
        ...
        Raises:
            ...
            VoiceprintRegistrationError: 声纹注册失败
            ValueError: 密码解密失败
        """
        # 1. 校验用户名唯一性
        ...
        try:
            ...
            logger.error("Gateway voiceprint enroll timeout: username=%s", username)
            raise VoiceprintRegistrationError("声纹注册超时，请稍后重试")
