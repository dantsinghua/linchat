...
class CaptchaService:
    ...
    async def generate(width: int = 120, height: int = 40) -> dict:
        """生成图形验证码，返回 {captcha_id, captcha_image}"""
    ...
    async def verify(captcha_id: str, captcha_code: str) -> bool:
        """验证码校验（一次性使用），失败抛 CaptchaInvalidException"""
...
class AuthService:
    ...
    async def login(
        ...
        """用户登录，返回 {token, user_id, username, expire_time}"""
        # 1. 验证码校验
        ...
        if user and user.is_locked():
            ...
            raise AccountLockedException(
                f"账户已锁定，请{remaining // 60 + 1}分钟后重试",
        ...
        await redis_setex_json(
            get_token_key(token_hash), settings.AUTH_TOKEN_IDLE_TTL, token_data
        )
