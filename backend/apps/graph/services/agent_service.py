...
from apps.graph.services.generation import register_generation, unregister_generation
...
from apps.graph.services.helpers import (
    build_prompt_preamble, create_first_token_messages,
    extract_content_control, extract_gateway_error, extract_usage,
    finalize_completion, finalize_execution, handle_execution_failure,
    handle_tool_end_event, init_langfuse, init_monitor_data,
    publish_monitor, push_final_monitor,
)
from apps.graph.services.cancel_monitor import monitor_cancel_signal
    ...
    async def execute(user_id: int, thread_id: str, request_id: str, user_message: str, attachment_uuids: Optional[list[str]] = None, channel: str = "web") -> AsyncGenerator[StreamChunk, None]:
        ...
        try:
            ...
            try:
                ...
                from apps.chat.repositories import message_repo as _mr
                hist = await _mr.find_latest_by_user(user_id, limit=getattr(settings, "CONTEXT_HISTORY_ROUNDS", 10) * 2)
                ctx = [{"role": m.role, "content": m.content} for m in hist]
                ctx.append({"role": "user", "content": user_message})
                if ContextService.check_token_limit(ctx, eff_win):
                    yield StreamChunk(type="context_compacting", content="")
                    await ContextService.compress_context(user_id=user_id, messages=ctx, effective_window=eff_win)
            ...
            async with create_chat_agent(prompt=preamble, preamble_tokens=ptokens, effective_window=eff_win) as agent:
                        ...
                        async for ev in agent.astream_events(input_message, config=config, version="v2"):
                            ...
                            if event_type == "on_chat_model_stream":
                                ...
                                if hasattr(chunk, "content") and chunk.content:
                                    ...
                                    sc = StreamChunk(type="content", content=chunk.content, message_id=assistant_msg.message_id if assistant_msg else None)
                                    if len(full_response) == len(chunk.content):
                                        sc.request_id = request_id
                                    yield sc
                            ...
                            try:
                                ...
                                if now - last_push >= interval and monitor_data is not None:
                                    monitor_data, last_alert = await publish_monitor(
                                        breakdown, max_ctx, model_name, mem_results,
                                        tool_procs, request_id, user_id,
                ...
                if not first_token_received:
                    finalize_execution(execution, LangGraphExecution.STATUS_FAILED, end, dur, error_type="NoTokenReceived", error_message="未收到任何响应")
                    await execution_repo.update(execution)
                    raise LLMInvalidResponseError("AI未返回任何响应，请重试")
                if interrupted:
                    await finalize_completion(execution, execution_repo, assistant_msg, message_repo, user_repo, full_response, end, dur, user_id, total_prompt_tokens, total_completion_tokens, langfuse_handler, interrupted=True)
                    yield StreamChunk(type="interrupted", content="[已中断]", message_id=assistant_msg.message_id)
                else:
                    await finalize_completion(execution, execution_repo, assistant_msg, message_repo, user_repo, full_response, end, dur, user_id, total_prompt_tokens, total_completion_tokens, langfuse_handler)
                    if monitor_data is not None:
                        await push_final_monitor(user_id, user_message, memory_modified, mem_results, breakdown, max_ctx, model_name, total_prompt_tokens, total_completion_tokens, tool_procs, request_id)
            ...
            if gateway_err:
                ...
                await handle_execution_failure(execution, execution_repo, start_time, f"GatewayError_{code}", msg, assistant_msg, message_repo, full_response or "")
                ...
                return
            await handle_execution_failure(execution, execution_repo, start_time, type(e).__name__, str(e), assistant_msg, message_repo, full_response or "")
            raise map_llm_exception(e)
        finally:
            if assistant_msg and assistant_msg.status == Message.STATUS_GENERATING:
                try:
                    end = timezone.now()
                    dur = int((end - start_time).total_seconds() * 1000)
                    assistant_msg.content = full_response or ""
                    assistant_msg.status = Message.STATUS_INTERRUPTED
                    assistant_msg.response_time_ms = dur
