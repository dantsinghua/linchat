...
| `search.py` | `SEARCH_TOOLS` | Brave Search API + Redis 限流 |
...
| 函数 | 参数 | 说明 |
|------|------|------|
| `web_search` | `query`, `num_results=5` | Brave Search API，Redis 限流（1次/秒/用户 + 2000次/月） |
...
django.conf         ← search.py, ha_client.py, ha_helpers.py（settings）
