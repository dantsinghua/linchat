    ...
    while elapsed < max_wait:
        ...
        if final_status != _prev_status or elapsed % 30 < poll_interval:
            logger.info(
                ...
                task_id, final_status, progress.get("current", "?"), progress.get("total", "?"), elapsed)
        ...
        if final_status == "failed":
            ...
            logger.warning("Doc parse failed: task=%s, err=%s, elapsed=%ds", task_id, err_msg, elapsed)
            return "failed", f"📄 {doc.file_name}: 解析失败 — {err_msg}", status_data
        if final_status == "incomplete":
            break
    else:
        logger.warning("Doc parse timeout: task=%s, last_status=%s, elapsed=%ds/%ds", task_id, final_status, elapsed, max_wait)
        await EventService.publish_event(
            user_id=user_id, event_type=evt,
