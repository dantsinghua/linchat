...
| `services/` | Agent 执行服务、helpers/ 包（prompt/errors/finalize/monitor）、取消监控、上下文压缩、GPU 锁、推理任务管理 |
| `subagents/` | SubAgent 子代理（搜索、记忆、代码、HA、多模态、文档解析）+ document_parse_helpers |
| `tools/` | Agent 工具集（搜索、记忆、代码执行、上下文、HA、历史搜索）+ user_id 公共工具 |
...
| `get_agent_config(user_id, callbacks)` | 构建 RunnableConfig（thread_id + `str(user_id)`，user_id 以字符串传递） |
...
各子模块均采用 `[前缀]` 结构化日志，覆盖全链路关键节点：

| 模块 | 日志前缀 | 关键节点 |
|------|----------|----------|
| `subagents/base.py` | `[SubAgent]` | START → LLM ready → tools → ainvoke → END OK / TIMEOUT / ERROR（含耗时、工具使用列表） |
| `subagents/document_agent.py` | `[DocParse]` | START → cache HIT/MISS → GPU lock → task created → poll done → result saved → DONE |
| `subagents/document_parse_helpers.py` | `[DocPoll]` | START → poll# → COMPLETED / FAILED / INCOMPLETE / TIMEOUT（含 poll_count、elapsed） |
| `tools/search.py` | `[web_search]` | START → rate_slot → Brave API → END（含 QPS 等待耗时、API 耗时、结果数） |
...
| 工具 | 策略 | 配置来源 |
|------|------|----------|
| `web_search` | QPS 等待式限流（超限等待最多 5s，而非拒绝）+ 月度配额耗尽才拒绝 | `settings.BRAVE_SEARCH_QPS` / `settings.BRAVE_SEARCH_MONTHLY_QUOTA` |
| `ha_control/query/diagnose` | 固定速率拒绝式限流（Redis TTL 60s） | `RATE_LIMITS` 常量 |
