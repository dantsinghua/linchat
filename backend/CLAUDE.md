...
| App | 关键模型 | 说明 |
|-----|----------|------|
| `chat` | Message, LangGraphExecution | 消息收发、SSE 流式响应、生成控制（停止/恢复/重连） |
...
| `voice` | SpeakerProfile, RegisteredDevice, VoiceSettings | WebSocket 语音流 → ASR 流式转录 → Agent Pipeline → TTS 流式合成、声纹、设备、ambient 环境监听、设备独占 |
...
| 文件 | 职责 |
|------|------|
| `settings.py` (509 行) | 全局配置：数据库、Redis(4 DB)、LLM 超时/重试、Gateway、MinIO、媒体限制、语音、认证、Memory、安全 |
