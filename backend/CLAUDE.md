...
| 文件 | 职责 |
|------|------|
| `settings/` 包 | 全局配置（batch-17 拆分）：`__init__.py` base（数据库、Redis 4 DB、LLM 超时/重试、Gateway、认证、Memory、安全）+ 聚合 `voice.py`（语音 active_conv 10s/LLM 阈值 0.75/HA_LAN_HOST + Channels）/`media.py`（MinIO 含 audio 桶、媒体限制）/`celery_conf.py` |
