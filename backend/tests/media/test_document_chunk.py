...
class TestChunkDocument:
    ...
    def test_empty_content(self):
        assert DocumentParseService.chunk_document("") == []

    def test_whitespace_only(self):
        assert DocumentParseService.chunk_document("   \n\n  ") == []

    def test_single_short_paragraph(self):
        result = DocumentParseService.chunk_document("Hello world.")
        assert result == ["Hello world."]

    def test_markdown_heading_split(self):
        """按 Markdown 标题拆分"""
        content = "# Section 1\n" + "A" * 100 + "\n\n# Section 2\n" + "B" * 100
        result = DocumentParseService.chunk_document(content, chunk_size=50)
        assert len(result) >= 2
        full = " ".join(result)
    ...
    def test_paragraph_split(self):
        ...
        result = DocumentParseService.chunk_document(content, chunk_size=200)
        assert len(result) >= 1
    ...
    def test_small_chunks_merged(self):
        ...
        content = "A\n\nB\n\nC\n\nD"
        result = DocumentParseService.chunk_document(content, chunk_size=100)
    ...
    def test_long_chunk_cut(self):
        ...
        content = "A" * 2000
        result = DocumentParseService.chunk_document(content, chunk_size=800, overlap=100)
        assert len(result) >= 3
...
量子计算在金融领域具有广阔的应用前景，但仍面临技术成熟度和人才储备等方面的挑战。"""
        result = DocumentParseService.chunk_document(content, chunk_size=100, overlap=20)
        assert len(result) >= 2
        full = " ".join(result)
        assert "引言" in full
        assert "结论" in full
