...
class TestSearchDocumentsRag:
    ...
    def test_keyword_search(self, mock_kw):
        ...
        results = run_async(DocumentParseService.search_documents_rag(user_id=42, query="量子计算", mode="keyword", limit=5))
    ...
    def test_semantic_search(self, mock_embed, mock_vec):
        ...
        results = run_async(DocumentParseService.search_documents_rag(user_id=42, query="量子", mode="semantic", limit=5))
    ...
    def test_hybrid_rerank(self, mock_embed, mock_vec, mock_kw):
        ...
        results = run_async(DocumentParseService.search_documents_rag(user_id=42, query="量子", mode="hybrid", limit=5))
        assert len(results) >= 1
    ...
    def test_empty_results(self, mock_embed, mock_vec, mock_kw):
        ...
        with patch("apps.media.repositories.media_attachment_repo.fulltext_search_parsed_content", new_callable=AsyncMock, return_value=[]):
            results = run_async(DocumentParseService.search_documents_rag(user_id=42, query="不存在的内容", mode="hybrid", limit=5))
        assert results == []
    ...
    def test_keyword_empty_no_indexed_docs(self, mock_kw):
        ...
        mock_kw.return_value = []
        with patch("apps.media.repositories.media_attachment_repo.fulltext_search_parsed_content", new_callable=AsyncMock, return_value=[]):
            results = run_async(DocumentParseService.search_documents_rag(user_id=42, query="test", mode="keyword", limit=5))
        assert results == []
