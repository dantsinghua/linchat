...
9. **Given** 用户成功发送聊天数据, **When** langgraph agent如果成功返回了，**Then** 此时的聊天框发送按钮变更为停止按钮，点击停止按钮则终止langgraph agent继续生成并生成一个checkpoint，已生成的消息显示"[已中断]"标记
...
| 流式响应中断 | 已接收内容保留显示，后端不记录不完整消息，弹出提示"响应已中断，如有需要请复制已显示内容" |
